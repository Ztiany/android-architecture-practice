...
suspend fun <T> FlowCollector<StateD<T>>.syncCallState(call: suspend () -> CallResult<T>) {
    emit(StateD.loading())
    call.invoke().intoStateDFlowCollector(this)
}
...
fun <T> (suspend () -> CallResult<T>).transformToStateFlow(): Flow<StateD<T>> {
    return flow {
        emit(StateD.loading())
        invoke().intoStateDFlowCollector(this)
    }
}
...
private suspend fun <T> CallResult<T>.intoStateDFlowCollector(collector: FlowCollector<StateD<T>>) {
    onSuccess {
