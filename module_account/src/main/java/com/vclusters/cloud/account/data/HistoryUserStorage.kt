...
class HistoryUserStorage @Inject constructor(
    ...
    fun historyUsers(): List<HistoryUser> {
        return historyUsersInternal().map {
            val decryptedPassword = AESUtils.decryptDataFromBase64(it.password, AESUtils.AES, appSettings.aesKey)
            it.copy(password = String(decryptedPassword ?: byteArrayOf(0)))
        }
    }

    fun deleteHistoryUser(user: HistoryUser): List<HistoryUser> {
        return historyUsers().toMutableListChecked().also {
            it.removeWhen { ele ->
    ...
    fun addHistoryUser(user: HistoryUser): List<HistoryUser> {
        return deleteHistoryUser(user).toMutableListChecked().also {
