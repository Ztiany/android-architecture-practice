package com.android.common.ui.pulltozoom;
...
public class PullToZoomScrollView extends NestedScrollView {
    ...
    private final int mZoomViewId;

    private View mContainerView;
    private final int mContainerViewId;

    private final int mMaxZoomHeight;
    private int mOriginContainerViewHeight;
    private int mOriginZoomViewHeight;
    private final int mDamp;
    private final float mZoomFactory;
        ...
        if (mContainerView.getHeight() <= mMaxZoomHeight && isTouchEvent) {
            if (deltaY < 0) {
                ...
                if (mContainerView.getHeight() - offset >= mOriginContainerViewHeight) {
                    int height = Math.min(mContainerView.getHeight() - offset, mMaxZoomHeight);
                    setContainerHeight(height);
                }
            } else {
                if (mContainerView.getHeight() > mOriginContainerViewHeight) {
                    int height = Math.max(mContainerView.getHeight() - deltaY, mOriginContainerViewHeight);
                    setContainerHeight(height);
                    return true;
                }
            }
    ...
    private void zoomView(int height) {
        ...
        mZoomView.setPivotX(measuredWidth / 2.0F);
